            ...
            <div>
              ...
              <form class="vx-row">
                <div class="vx-col md:w-1/3 w-full mb-2">
                  <label>{{$t('Profile.password.old')}}<span class="text-danger">*</span></label>
                  <vs-input
                    class="w-full"
                    type="password"
                    :name="`Old Password`"
                ...
                <div class="vx-col md:w-1/3 w-full mb-2">
                  <label>{{$t('Profile.password.new')}}<span class="text-danger">*</span></label>
                  <vs-input
                    class="w-full"
                    type="password"
                    :name="`New Password`"
                ...
                <div class="vx-col md:w-1/3 w-full mb-2">
                  <label
                    >{{$t('Profile.password.confirm')}}d<span class="text-danger"
                      >*</span
                    ></label
                  >
                  <vs-input
...
export default {
  ...
  methods: {
    ...
    uploadFile(event) {
      ...
      event.target.value = '';
      /* this.$vs.loading({
        container: '#display-profile-upload',
        scale: 0.45
      });
      this.$store
        .dispatch('studio/uploadMedia', selectedFile)
        .then(url => {
          this.userInfo = { ...this.userInfo, profile_image: url };
        })
        .catch(() => {
          this.$vs.notify({
            title: 'Error Occured',
        ...
        .finally(() => {
          document.getElementById(`display-profile-input`).value = '';
          this.uploadInProgress = false;
          this.$vs.loading.close('#display-profile-upload > .con-vs-loading');
        }); */
    },
